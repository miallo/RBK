...
\begin{figure}[!ht]
	...
	\caption{Temperaturprofile in Abhängigkeit der Höhe.}
	\label{fig:t_num}
    \end{subfigure}
    ~
...
Die andere Grenze der Geschwindigkeit ist die viskose Endgeschwindigkeit:
\begin{align*}
	v_\text{End} = <+++++++++++++++++++++++++++++++++++++++++++++++++++++++++>\quad.% \frac{\nu}{L}\approx5\times10^{-2}\si{\centi\meter\per\second}
\end{align*}
Sie gibt an, welche Endgeschwindigkeit basierend auf der Reibung erreicht wird.
...
Und zum anderen die durch \textsc{Stokes}-Reibung verringerte Geschwindigkeit, die aus dem Gleichsetzen der Stokesschen Reibungskraft mit der Auftriebskraft kommt:
\begin{align*}
  v_\text{Stokes} = \frac{2}{9}\frac{\left( \rho_\text{Plume}-\rho_\text{Fluid} \right)}{\eta}gR^2=\frac{2}{9}\frac{gR^2\left( 1-\alpha\Delta T \right)}{\nu}\approx
\end{align*}<++>
